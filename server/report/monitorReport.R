 ...
 teamStatREPORT <-enumStatREPORT %>% 
        group_by(regionCode, RegionName, districtCode, districtName, townCity, 
                 borderPostName, team_number
                 ) %>%
        summarise(
 ...
 borderStatREPORT <-teamStatREPORT %>% 
   group_by(regionCode, RegionName, districtCode, districtName, townCity, 
            borderPostName
   ) %>%
   summarise(
 ...
 districtStatREPORT <-borderStatREPORT %>% 
   group_by(regionCode, RegionName, districtCode, districtName
   ) %>%
   summarise(
     `total Respondents` = sum(`total Respondents`),
     `total IN_MIGRATION(X)` = sum(`total IN_MIGRATION(X)`),
 ...
 regionStatREPORT <- districtStatREPORT %>% 
